...
class ModularMonolithDirettoreApplication[InputT, TraceT]:
    """Modular-monolith facade backed by an already configured slot provider."""

    def __init__(
        self,
        *,
        slot_provider: ExecutionSlotProvider[
            ModularMonolithExecutionSlot[InputT, TraceT], InputT, TraceT
        ],
    ) -> None:
    ...
    ) -> ModularMonolithExecutionSlot[InputT, TraceT]:
    ...
    async def transactional_slot(
        self, *, saga_id: str | None = None
    ) -> AsyncGenerator[ModularMonolithExecutionSlot[InputT, TraceT]]:
